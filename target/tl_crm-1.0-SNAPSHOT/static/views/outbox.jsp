<%-- 
    Document   : outbox
    Created on : Dec 7, 2016, 8:33:28 AM
    Author     : paulodondi
--%>
...
<div class="page-title">                    
    <h2><span class="fa fa-arrow-circle-o-left"></span> Outbox Messages</h2>
</div>
<!-- END PAGE TITLE --> 
<div class="row" ng-controller="outboxController">
    <div class="col-md-12">
        <!-- START DATATABLE EXPORT -->
        <div class="panel panel-default" ng-if="showGridView">
            <div class="panel-heading">
                <h3 class="panel-title">Outbox Messages Listing</h3>
                ...
                <table id="customers2" ng-table="tableParams" class="table table-bordered" show-filter="true">
                    <tbody>
                        <tr ng-repeat="item in $data" id="{{ item.id }}">
                            ...
                            <td title="'ShortCode'" filter="{ shortCode: 'text'}" sortable="'shortCode'">{{item.shortCode}}</td>
                            <td title="'Organization'" filter="{ orgName: 'text'}" sortable="'orgName'">{{item.orgId.orgName}}</td>
                            <td title="'Delivery Status'" filter="{ deliveryStatus: 'text'}" sortable="'deliveryStatus'">{{item.deliveryStatus}}</td>
                            <td title="'Date Sent'" filter="{ dateSent: 'text'}" sortable="'dateSent'">{{item.dateSent}}</td>
                            <td title="'Delivery Time'" filter="{ deliveryTime: 'text'}" sortable="'deliveryTime'">{{item.deliveryTime}}</td>
                        </tr>
                    </tbody>
                </table>                                    
            </div>
